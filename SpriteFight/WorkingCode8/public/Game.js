...
function draw(data){
  ...
  context.drawImage(bck, 0, 0 ,canvsize.width, canvsize.height)  
  if(data[ind].canAttack){
    AtkImage.src = "Attack.png"
  } else if (!data[ind].canAttack){
    AtkImage.src = "noAttack.png"
  }
  context.drawImage(AtkImage,50,canvsize.height-125,100,100);
...
// Draw the character and flip him if necessary 
      ...
      context.drawImage(image,cutx-1,cuty,data[i].width,data[i].height,data[i].dir?-data[i].width:0,0,data[i].width,data[i].height);
